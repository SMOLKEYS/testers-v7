...
const testerjs = extend(MessageBlock, "testerjs", {
});
...
testerjs.buildType = () => {
  return extend(MessageBlock.MessageBuild, testerjs, {
    _haserr: false,
    _err: "",
    _crit: false,
...
const testerdraw = extend(MessageBlock, "testerdraw", {
});

testerdraw.buildType = () => {
  return extend(MessageBlock.MessageBuild, testerdraw, {
    _haserr: false,
    _err: "",
...
const testerdrawclear = extend(MessageBlock, "testerdrawclear", {
});

testerdrawclear.buildType = () => {
  return extend(MessageBlock.MessageBuild, testerdrawclear, {
    _haserr: false,
    _err: "",
...
const testertable = extend(MessageBlock, "testertable", {
  init(){
    this.super$init();
...
testertable.buildType = () => {
  return extend(MessageBlock.MessageBuild, testertable, {
    _haserr: false,
    _err: "",
    _buildmode: false,
    buildConfiguration(table){
...
const testerfx = extend(MessageBlock, "testerfx", {
});
...
let amusma = new Integer(1)
let amasmu = new Integer(2)

testerfx.buildType = () => {
  return extend(MessageBlock.MessageBuild, testerfx, {
    _haserr: false,
    _err: "",
    _long: false,
    buildConfiguration(table){
      ...
      const lbutton = table.button((this._long)?Icon.commandRally:Icon.commandRallySmall, () => {
        ...
        this.configure(amasmu);
      }).size(40).get();
      table.button(Icon.star, () => {
        this.configure(amusma);
      }).size(40);
    },
    ...
    updateTableAlign(table){
      if(!this._haserr){
        this.super$updateTableAlign(table);
      }else{
        const pos = Core.input.mouseScreen(this.x + Vars.tilesize / 2 + 1, this.y);
        table.setPosition(pos.x, pos.y, Align.left);
      }
